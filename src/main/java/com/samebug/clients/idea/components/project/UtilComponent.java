                        ...
                        public void run() {
                            try {
                                ToolWindowManager.getInstance(myProject).notifyByBalloon(
                                        ...
                                        SamebugNotifications.basicHyperlinkListener(myProject));
                                pluginState.setFirstRun(false);
                            } catch (IllegalStateException e1) {
                                LOGGER.warn("Samebug tool window was not initialized after " + DELAY_MS + " millis, welcome message could not be displayed", e1);
                            } catch (Exception e2) {
